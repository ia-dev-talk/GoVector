...
_GPS_NUMBER_RE = re.compile(r"(?<!\d)[+-]?\d{1,3}(?:[.,]\d+)?(?!\d)")
...
def _select_gps_coordinates(
    ...
    valid_coordinates = []
    warnings = []
    for source in ("GPS_PCO", "GPS_DERIVATION", "GPS_SPLITTER"):
        raw_value = gps_values.get(source)
        if _clean(raw_value) is None:
            continue
        coordinates = _parse_gps(raw_value)
        if coordinates is None:
            warnings.append(f"{source} invalide ou ambigu : coordonnées ignorées.")
            continue
    ...
    divergent_sources = [
        ...
        if abs(coordinates[0] - latitude) > 1e-6
        or abs(coordinates[1] - longitude) > 1e-6
    ]
    if divergent_sources:
        sources = ", ".join([selected_source, *divergent_sources])
        warnings.append(
            f"Coordonnées GPS divergentes ({sources}) : {selected_source} retenu."
        )
...
def build_job_record(
    ...
    Source spreadsheets are evidence, not an authorization to invent operational
    values. In particular this function never creates a job type, required skill,
    assignment, reference or GPS coordinate that was not explicitly present in a
    trusted mapped source column.
    """
    ...
    if not service_city and sector_raw:
        import_warnings.append("Ville absente : secteur Excel conservé séparément.")
    ...
    # Profiles may still govern presentation/default priority, but never create
    # a type or skill requirement for an imported intervention.
    ...
    # COMMANDE is the operational reference in the MAGILLAN workbook. REFERENCE
    # remains a supported explicit alternative, but no synthetic reference is
    # generated from NRO/PBO/row numbers because that would break truthfulness and
    # idempotence on repeated imports.
    job_number = n_com or reference
