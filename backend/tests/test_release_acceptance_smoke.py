...
def test_stock_contract_failure_is_explicit(monkeypatch):
    ...
    with pytest.raises(module.SmokeFailure, match="stock technicien"):
        module.run(_args(admin_token=None, dataset_id=None))
...
def test_strict_delivery_requires_all_inputs():
    with pytest.raises(module.SmokeFailure, match="paramètres manquants"):
        module.run(
            _args(
