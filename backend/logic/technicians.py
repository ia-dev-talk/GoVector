	...
	**kwargs,
	...
	for field, value in kwargs.items():
		...
		if hasattr(tech, model_field):
			...
			setattr(tech, model_field, value)
...
def _map_to_live_status(status: TechnicianStatus) -> TechnicianLiveStatus:
	"""Convertit l'ancien status vers le nouveau live_status"""
	mapping = {
		TechnicianStatus.AVAILABLE: TechnicianLiveStatus.DISPONIBLE,
		TechnicianStatus.ON_JOB: TechnicianLiveStatus.EN_INTERVENTION,
		TechnicianStatus.EN_ROUTE: TechnicianLiveStatus.EN_INTERVENTION,
		TechnicianStatus.ON_BREAK: TechnicianLiveStatus.PAUSE,
		TechnicianStatus.OFF_DUTY: TechnicianLiveStatus.HORS_SERVICE,
	}
	return mapping.get(status, TechnicianLiveStatus.DISPONIBLE)
...
async def update_technician_status(
	...
	"""Update technician's live_status (source de vérité unique)"""
	tech = await get_technician(db, tech_id)
	if not tech:
		return None

	tech.live_status = status
	tech.updated_at = datetime.utcnow()
...
async def get_technicians_by_orienteur(
    db: AsyncSession,
    orienteur_id: int,
    skip: int = 0,
    limit: int = 100,
...
async def get_available_technicians_by_orienteur(
    db: AsyncSession,
    orienteur_id: int
) -> List[Technician]:
    """Get available technicians for a specific orienteur"""
    result = await db.execute(
        select(Technician).where(
            Technician.is_active == True,
            Technician.status == TechnicianStatus.AVAILABLE,
