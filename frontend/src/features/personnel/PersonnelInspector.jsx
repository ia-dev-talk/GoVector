...
const TABS = Object.freeze([
  ...
  { id: 'skills', label: 'Compétences', Icon: ToolIcon },
  { id: 'equipment', label: 'Équipement', Icon: SettingsIcon },
...
function normalizeForm(tech) {
  return {
    ...
    address: text(tech?.home_address ?? tech?.address),
    primary_sector_id: tech?.primary_sector_id
    ...
    live_status: normalizeStatus(tech?.live_status),
  ...
  return (
    <div
      className={[
        ...
        tone ? `personnel-v3-inspector-metric--${tone}` : '',
      ].filter(Boolean).join(' ')}
    >
      <span>{label}</span>
      <strong>{value}</strong>
...
function JobRow({ job, onOpen, withDate = false }) {
  const reference = text(
    job?.dtli,
    `Intervention ${job?.job_number ?? job?.id ?? '—'}`,
  );
  const customer = text(job?.customer_name, 'Client non renseigné');
  const status = statusLabel(job?.status);
  const date = withDate
    ? formatDateTime(job?.updated_at ?? job?.completed_at ?? job?.scheduled_date)
    : '';
  const content = (
    <>
      <span className="personnel-v3-job-card-main">
        <strong>{reference}</strong>
        <span>{customer}</span>
      </span>
      <span className="personnel-v3-job-card-meta">
        <small>
          {status}
          {date ? ` · ${date}` : ''}
        </small>
        {typeof onOpen === 'function' ? <ChevronIcon /> : null}
      </span>
    </>
  );

  if (typeof onOpen !== 'function') {
    return <article className="personnel-v3-job-card">{content}</article>;
  }

  return (
    <button
      type="button"
      className="personnel-v3-job-card personnel-v3-job-card--interactive"
      onClick={() => onOpen(job)}
      title={`Ouvrir ${reference}`}
    >
      {content}
    </button>
  );
...
function PersonnelInspectorContent({
  ...
  const [form, setForm] = useState(() => normalizeForm(tech));
  ...
  const techJobsToday = useMemo(
    () => (Array.isArray(todayJobs) ? todayJobs : []),
    [todayJobs],
  );
  ...
  const gpsAge = getLocationAgeMinutes(tech, referenceNow);
  const currentStatus = normalizeStatus(tech.live_status);
  const statusChanged =
    form.live_status && form.live_status !== currentStatus;
  const canSave = canEditGeneral || statusChanged;

  const normalizedSectors = useMemo(
    () => Array.isArray(sectors)
      ? sectors.filter((sector) => Number.isInteger(Number(sector?.id)))
      : [],
    [sectors],
  );
  ...
  const setPrimarySector = (value) => {
    ...
    setForm((current) => {
      if (!normalizedValue) {
        return { ...current, primary_sector_id: '' };
      }
  ...
  const toggleSector = (sectorId) => {
    setForm((current) => {
      const exists = current.sector_ids.includes(sectorId);
      if (exists) {
        const sectorIds = current.sector_ids.filter(
        ...
        return {
          ...
          sector_ids: sectorIds,
          primary_sector_id:
            String(sectorId) === current.primary_sector_id
              ? ''
      ...
      return {
        ...current,
        sector_ids: [...current.sector_ids, sectorId],
      };
    });
  };
  ...
  const handleTabChange = async (nextTab) => {
    ...
    try {
      const response = await api.getJobs({ assigned_tech_id: tech.id });
      setHistoryJobs(
  ...
  const handleSave = async () => {
    ...
    try {
      await onSave(tech, {
        ...
        primary_sector_id: form.primary_sector_id
      ...
      <div className="personnel-v3-inspector-body">
          ...
          <>
            <section className="personnel-v3-inspector-section">
              ...
              <div className="personnel-v3-inspector-metrics">
                <Metric label="Interventions" value={techJobsToday.length} />
                <Metric
                  label="Terminées"
              ...
              <div className="personnel-v3-form-grid">
                ...
                <Field label="Statut terrain">
                  <select
                    ...
                    onChange={(event) => updateForm('live_status', event.target.value)}
                  >
                ...
                <Field label="Orienteur">
                  <input value={text(tech.orienteur_name, 'Non affecté')} disabled />
                </Field>
                <Field label="Équipe">
                  <input value={text(tech.team, 'Liée à l’orienteur')} disabled />
                </Field>
                <Field label="Secteur principal" wide>
                  <select
                    value={form.primary_sector_id}
                    disabled={!canEditGeneral || normalizedSectors.length === 0}
                    onChange={(event) => setPrimarySector(event.target.value)}
                  >
                    <option value="">Aucun secteur principal</option>
                    {normalizedSectors.map((sector) => (
                      <option key={sector.id} value={sector.id}>
                        {text(sector.name, `Secteur ${sector.id}`)}
                        {sector.is_active === false ? ' — inactif' : ''}
                      </option>
                    ))}
                  </select>
                  {normalizedSectors.length === 0 ? (
                    <small className="personnel-v3-field-help">
                      Aucun secteur configuré. Créez d’abord les secteurs dans le module Secteurs.
                    </small>
                  ) : null}
                </Field>
                <Field label="Début de service">
                  <input
                    ...
                    disabled={!canEditGeneral}
                    onChange={(event) => updateForm('shift_start', event.target.value)}
                  />
                </Field>
                <Field label="Fin de service">
                  <input
                    ...
                    disabled={!canEditGeneral}
                    onChange={(event) => updateForm('shift_end', event.target.value)}
                  />
                </Field>
                <Field label="Maximum quotidien">
                  <input
                    ...
                    disabled={!canEditGeneral}
                    onChange={(event) => updateForm('max_jobs_per_day', event.target.value)}
                  />
                </Field>
              ...
              {normalizedSectors.length > 0 ? (
                <div className="personnel-v3-sector-picker">
                  {normalizedSectors.map((sector) => {
                    ...
                    const checked = selectedSectorIds.has(String(sectorId));
                    return (
                      <label
                        key={sector.id}
                        className={[
                          ...
                          checked ? 'is-selected' : '',
                          sector.is_active === false ? 'is-inactive' : '',
                        ].filter(Boolean).join(' ')}
                      >
                        <input
                          type="checkbox"
                        ...
                        <span
                          ...
                          style={{ backgroundColor: text(sector.color, '#4b8dff') }}
                          aria-hidden="true"
                        />
                        <span>{text(sector.name, `Secteur ${sector.id}`)}</span>
                        {String(sectorId) === form.primary_sector_id
                ...
                <div className="personnel-v3-section-empty">Aucun secteur configuré.</div>
              )}
              <small className="personnel-v3-field-help">
                Le secteur principal est automatiquement inclus dans les secteurs affectés.
              </small>
            </div>
          </section>
        ) : null}
        ...
        {tab === 'equipment' ? (
          <section className="personnel-v3-inspector-section">
            ...
            <div className="personnel-v3-linked-data-note">
              ...
              <span>
                Le matériel affecté doit rester traçable depuis le dépôt jusqu’au technicien,
                puis jusqu’à l’intervention où il est consommé ou installé.
              </span>
              {typeof onOpenStock === 'function' ? (
                <button
                  type="button"
                  className="personnel-v3-linked-action"
                  onClick={() => onOpenStock(tech)}
                >
                  Ouvrir le stock du technicien
                  <ChevronIcon />
                </button>
              ) : null}
            </div>
            <div className="personnel-v3-equipment-grid">
              ...
              <Metric
                label="Câble"
                value={tech.cable_length_m != null ? `${tech.cable_length_m} m` : '—'}
              />
            ...
            {techJobsToday.length > 0 ? (
              <div className="personnel-v3-job-list">
                {techJobsToday.map((job) => (
                  <JobRow key={job.id} job={job} onOpen={onOpenJob} />
                ))}
              </div>
            ) : (
        ...
        {tab === 'history' ? (
          <section className="personnel-v3-inspector-section">
            ...
            {historyLoading ? (
              <div className="personnel-v3-section-empty">Chargement de l’historique…</div>
              ...
              <div className="personnel-v3-section-empty">Aucun historique disponible.</div>
            )}
          </section>
        ) : null}
      </div>
...
export default function PersonnelInspector({
  ...
  if (!tech) {
    return <EmptyInspector selectedCount={selectedCount} />;
  }
