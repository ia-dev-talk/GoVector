...
function NavIcon({ name }) {
  ...
  switch (name) {
    ...
    case 'jobs':
      return (
        <svg {...commonProps}>
          ...
          <path d="M7 7h6M7 10h6M7 13h4M7 1.8h6v2.4H7z" />
        </svg>
      );
    case 'planning':
      return (
    ...
    case 'reports':
      return (
        <svg {...commonProps}>
          <path d="M3 17V9M8 17V5M13 17V9M18 17V2M2 17.5h17" />
        </svg>
      );
    case 'map':
      return (
    ...
    case 'supervision':
      return (
        <svg {...commonProps}>
          ...
          <path d="M7 17h6M10 14v3M5 10l2.5-2.5 2 1.8 3.5-3.8 2 2" />
        </svg>
      );
    case 'sectors':
      return (
        <svg {...commonProps}>
          <path d="M2.5 9.5 10 3l7.5 6.5M4 8.5V17h12V8.5M8 17v-5h4v5" />
        </svg>
      );
    case 'settings':
      return (
...
export default function AppLayout({
  ...
  const role = ROLE_CONFIG[normalizedRole] || {
  ...
  const visibleNavItems = useMemo(
    () => NAV_ITEMS.filter((item) => item.roles.includes(normalizedRole)),
    [normalizedRole],
  );

  const activeItem = useMemo(
    () =>
      visibleNavItems.find((item) => item.id === normalizedCurrentPage) || null,
    [normalizedCurrentPage, visibleNavItems],
  );
  ...
  const handleNavigate = useCallback(
    (pageId) => {
      ...
      const navigated = onNavigate(pageId);
      if (isMobile && navigated !== false) setMobileMenuOpen(false);
    },
    [isMobile, onNavigate],
  );

  const sidebarVisible = !isMobile || mobileMenuOpen;
  const sidebarIsExpanded = isMobile || sidebarExpanded;
  const sidebarWidth = isMobile ? 'min(86vw, 300px)' : sidebarExpanded ? 220 : 52;

  return (
    <div
      className={`app-layout ${sidebarIsExpanded ? 'sidebar-open' : 'sidebar-closed'}`}
    ...
    >
      ...
      {sidebarVisible && (
        ...
        >
          <div
            ...
            style={{
              gap: 'var(--space-sm)',
              justifyContent: sidebarIsExpanded ? 'space-between' : 'center',
            }}
          >
            {sidebarIsExpanded ? (
              <div
            ...
            {sidebarIsExpanded && (
              <button
                ...
                aria-label={isMobile ? 'Fermer la navigation' : 'Réduire la navigation'}
                title={isMobile ? 'Fermer' : 'Réduire la navigation'}
              >
                <NavIcon name={isMobile ? 'close' : 'collapse'} />
              </button>
            )}
          </div>
          ...
          <div
            ...
            style={{
              ...
              justifyContent: sidebarIsExpanded ? 'flex-start' : 'center',
            }}
            title={role.label}
          >
            <span
            ...
            {sidebarIsExpanded && (
              <span className="sidebar-role-label">{role.label}</span>
            )}
          </div>
            ...
            {visibleNavItems.map((item) => {
              ...
              return (
                <button
                  ...
                  title={sidebarIsExpanded ? undefined : item.label}
                  style={{
                    justifyContent: sidebarIsExpanded ? 'flex-start' : 'center',
                    minHeight: isMobile ? 48 : 36,
                    padding: sidebarIsExpanded ? undefined : 'var(--space-md)',
                  }}
                >
          ...
          <div className="sidebar-footer">
            <button
              ...
              title={sidebarIsExpanded ? undefined : 'Déconnexion'}
              style={{
                justifyContent: sidebarIsExpanded ? 'flex-start' : 'center',
                minHeight: isMobile ? 48 : 36,
                padding: sidebarIsExpanded ? undefined : 'var(--space-md)',
      ...
      <main id="bluevector-main-content" className="app-main">
        {isMobile && (
          <header
            style={{
              minHeight: 48,
