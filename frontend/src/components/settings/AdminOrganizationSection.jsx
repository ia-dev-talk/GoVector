...
export default function AdminOrganizationSection({
  ...
  const createClient = async (event) => {
    ...
    try {
      ...
      form.reset();
      await load();
      toast?.('Entreprise cliente créée.', 'success');
    } catch (error) {
  ...
  const createClientAccount = async (event) => {
    ...
    try {
      ...
      form.reset();
      toast?.('Compte client lecture seule créé.', 'success');
    } catch (error) {
      toast?.(message(error), 'error');
    }
  };
  ...
  const createTeam = async (event) => {
    ...
    const form = event.currentTarget;
    const sectorIds = [
      ...form.querySelectorAll('input[name="sector_ids"]:checked'),
  ...
  const assignTechnician = async (teamId, technicianId, grade) => {
    try {
      ...
      await load();
      toast?.('Technicien déplacé dans l’équipe.', 'success');
    } catch (error) {
      toast?.(message(error), 'error');
    }
  };
  ...
  const updateTeamConfiguration = async (event, teamId) => {
    ...
    try {
      ...
      await load();
      toast?.('Configuration de l’équipe mise à jour.', 'success');
    } catch (error) {
      toast?.(message(error), 'error');
    }
  };
  ...
  const removeTechnician = async (teamId, technicianId) => {
    try {
      ...
      await load();
      toast?.('Technicien retiré de l’équipe.', 'success');
    } catch (error) {
      toast?.(message(error), 'error');
    }
  };
  ...
  return (
    <div className="v1-admin-grid">
      {isAdmin ? (
        <section className="v1-admin-card">
          <header>
            <span>Donneurs d’ordre</span>
          ...
          <div className="v1-admin-list">
            {clients.map((client) => (
              <div key={client.id}>
                ...
                <span>{client.code} · {client.operator || 'opérateur non défini'}</span>
                <details className="v1-admin-inline-editor">
                  <summary>Modifier</summary>
                  <form
                    onSubmit={(event) => updateClientConfiguration(event, client.id)}
                    className="v1-admin-form"
                  >
                    <input
                      name="name"
                      required
                    ...
                    <input
                      ...
                      aria-label="Code de l’entreprise"
                    />
                    <input
                      name="operator"
                      defaultValue={client.operator || ''}
                      placeholder="Opérateur associé"
                      aria-label="Opérateur associé"
                    />
                    <button type="submit">Enregistrer</button>
                  </form>
                </details>
                <button type="button" onClick={() => toggleClient(client)}>
                  {client.is_active ? 'Archiver / couper l’accès' : 'Réactiver'}
                </button>
              </div>
            ))}
          </div>
      ...
      {isAdmin ? (
        <section className="v1-admin-card">
          <header>
            ...
            <h2>Compte client lecture seule</h2>
          </header>
      ...
      <section className="v1-admin-card v1-admin-card--wide">
        <header>
          ...
          <h2>Équipes, orienteurs et secteurs</h2>
        </header>
        <form onSubmit={createTeam} className="v1-admin-form v1-admin-form--team">
          ...
          <select name="orienteur_id" required defaultValue="">
            <option value="" disabled>Orienteur unique</option>
            {orienteurs.map((item) => (
              <option key={item.id} value={item.id}>{item.name}</option>
            ))}
          </select>
          <select name="initial_technician_id" required defaultValue="">
            <option value="" disabled>Premier technicien</option>
            {technicians.map((item) => (
              <option key={item.id} value={item.id}>{item.name}</option>
            ))}
          </select>
          <select name="initial_grade" required defaultValue="junior">
            <option value="junior">Technicien junior</option>
            <option value="senior">Technicien senior</option>
          </select>
          <fieldset>
            <legend>Un ou plusieurs secteurs</legend>
            {sectors.map((sector) => (
              <label key={sector.id}>
        ...
        <div className="v1-admin-team-grid">
          {teams.map((team) => (
            <article key={team.id}>
              <h3>{team.name}</h3>
              <p>{team.orienteur_name} · {team.sector_names.join(', ')}</p>
              <details className="v1-admin-inline-editor">
                <summary>Modifier l’équipe</summary>
                <form
                  onSubmit={(event) => updateTeamConfiguration(event, team.id)}
                  className="v1-admin-form v1-admin-form--team-edit"
                >
                  <input
                    name="name"
                    required
                  ...
                  <select
                    name="orienteur_id"
                    required
                    defaultValue={String(team.orienteur_id)}
                    aria-label="Orienteur de l’équipe"
                  >
                    {orienteurs.map((item) => (
                      <option key={item.id} value={item.id}>{item.name}</option>
                    ))}
                  ...
                  <fieldset>
                    <legend>Secteurs de l’équipe</legend>
                    {sectors.map((sector) => (
                      <label key={`${team.id}-${sector.id}`}>
                        <input
                          type="checkbox"
                          name="sector_ids"
                          value={sector.id}
                          defaultChecked={team.sector_ids.includes(sector.id)}
                        />
                        {sector.name}
                      </label>
                    ))}
              ...
              <div className="v1-admin-chip-row">
                {team.technicians.map((tech) => (
                  <span key={tech.id}>
                    {tech.name} · {tech.grade}
                    <button
                      type="button"
                      aria-label={`Retirer ${tech.name}`}
                      onClick={() => removeTechnician(team.id, tech.id)}
                    >
                      ×
                    </button>
                  </span>
              ...
              <div className="v1-admin-team-add">
                <select
                  ...
                  defaultValue=""
                  onChange={(event) => {
                    const [id, grade] = event.target.value.split(':');
                    if (id) {
                      assignTechnician(team.id, Number(id), grade);
                    }
                    event.target.value = '';
                  }}
                >
                  ...
                  {technicians.map((tech) => (
                    <option key={`${team.id}-${tech.id}-j`} value={`${tech.id}:junior`}>
                      {tech.name} · junior
                    </option>
                  ))}
                  {technicians.map((tech) => (
                    <option key={`${team.id}-${tech.id}-s`} value={`${tech.id}:senior`}>
                      {tech.name} · senior
                    </option>
                  ))}
                </select>
                <button
                  type="button"
                  className="v1-admin-quiet-button"
                  onClick={() => toggleTeam(team)}
                >
                  {team.is_active ? 'Archiver l’équipe' : 'Réactiver l’équipe'}
                </button>
              </div>
            </article>
          ))}
